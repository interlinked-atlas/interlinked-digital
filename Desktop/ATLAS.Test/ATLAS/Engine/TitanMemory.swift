...
class TitanMemory {
    ...
    func syncFromCloud() {
        Task.detached(priority: .background) {
            guard let url = URL(string: "\(SupabaseConfig.projectURL)/rest/v1/titan_memory?select=id,product_name,match_patterns,steps,hosts_entries,platform") else { return }
    ...
    func fetchAllEntries() async -> [[String: Any]] {
        guard let url = URL(string: "\(SupabaseConfig.projectURL)/rest/v1/titan_memory?select=id,product_name,file_name,match_patterns,steps,hosts_entries,confirmed_by,confirmed_at,install_log,platform") else { return [] }
        var req = URLRequest(url: url)
        req.setValue(SupabaseConfig.anonKey, forHTTPHeaderField: "apikey")
